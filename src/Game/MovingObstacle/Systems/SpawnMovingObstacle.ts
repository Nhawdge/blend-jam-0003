...
export default function SpawnMovingObstacle(update: Update) {
  ...
  const movingObstacles = entities.entityInstances.filter(x => x.__identifier == 'MovingObstacle').filter(x => x?.fieldInstances[0]?.__value?.cx)!; // TODO get the entitiy name from ldtk
  //console.log(movingObstacles);
